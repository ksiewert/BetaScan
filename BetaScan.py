...
def main():
	...
	for SNPi in range(len(SNPs)):
		...
		if freq<1.0 and freq>0:
			...
			#This will be inefficient. Has to go through entire array every time
			sI,endI = find_win_indx(prevStarti, prevEndi, SNPi, SNPs, args.w)
			#SNPSet_Is = np.where(np.logical_and(SNPLocs>=core_loc-args.w/2., SNPLocs<=core_loc+args.w/2.)) #obtain only SNPs within the core SNP's window
			B = 0
			#pdb.set_trace()
			if endI>sI:
